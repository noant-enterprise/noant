...
export default function CustomersPage() {
  ...
  const { users, loading } = useUsers(search, plan)

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-text-primary">Customers</h1>
        <p className="text-sm text-text-tertiary">{users.length} total users</p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-bg-surface overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-sm text-text-tertiary">Loading...</td>
              </tr>
            ) : users.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-8 text-center text-sm text-text-tertiary">No users found</td>
              </tr>
            ) : (
              users.map(user => (
