...
func trimTurnText(text string, max int) string {
...
func (b *AIBrain) recentConversationTurns(ctx context.Context, conversationID, currentQuery string, limit int) []MessageTurn {
	...
	for _, msg := range messages {
		...
		turns = append(turns, MessageTurn{
			Role:    normalizeTurnRole(msg.Role),
...
func (b *AIBrain) buildSalesCoachMessage(conv *domain.Conversation, user *domain.User, query string, history []MessageTurn, inventory []domain.InventoryItem, qaPairs []domain.QAPair) MessageTurn {
	...
	if user != nil {
		...
		if whatsapp, _ := b.repos.User.GetOwnerWhatsApp(context.Background(), user.ID); whatsapp != "" {
	...
	if len(inventory) > 0 {
		...
		for i, item := range inventory {
			...
			if item.MinPrice != nil && *item.MinPrice > 0 {
				minPrice = fmt.Sprintf(" min=%.0f", *item.MinPrice)
			}
			parts = append(parts, fmt.Sprintf("%s (%.0f%s%s)", item.Name, item.Price, stock, minPrice))
		}
