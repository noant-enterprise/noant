...
describe('OfflineBanner', () => {
  it('shows offline banner when offline', () => {
    mockUseNetwork.mockReturnValue({ isOnline: false, wasOffline: false, isServerDown: false, wasServerDown: false, lastChecked: null })
  ...
  it('returns null when online and was never offline', () => {
    mockUseNetwork.mockReturnValue({ isOnline: true, wasOffline: false, isServerDown: false, wasServerDown: false, lastChecked: null })
    const { container } = render(<OfflineBanner />)
    expect(container.innerHTML).toBe('')
  })

  it('shows back online banner after reconnecting', () => {
    mockUseNetwork.mockReturnValue({ isOnline: true, wasOffline: true, isServerDown: false, wasServerDown: false, lastChecked: new Date() })
    render(<OfflineBanner />)
